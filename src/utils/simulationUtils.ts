...
export function useStoreCode() {
  ...
  return useCallback(
    (
      addressAndFunds: AddressAndFunds,
      file: { filename: string; fileContent: Buffer | JSON }
    ) => {
      let codeId: number;
      setSimulateApp(({ app }) => {
        codeId = app.wasm.create(
...
export function useInstantiateContract() {
  ...
  return useCallback(
    async (
      ...
      const result = await app.wasm.instantiateContract(
        sender,
        funds,
        codeId,
        instantiateMsg,
...
export function useExecute() {
  ...
  return useCallback(
    async (
      ...
      const result = await app.wasm.executeContract(
        sender,
        funds,
        contractAddress,
        executeMsg,
...
export function useExecuteSubMsg() {
  ...
  const [trace, setTrace] = useAtom(traceState);
  return useCallback(
    async (contractAddress: string, message: any) => {
      const result = await app.wasm.executeSubmsg(
        contractAddress,
        message,
        trace
      );
...
export function useReply() {
  const [{ app }, setSimulateApp] = useAtom(cwSimulateAppState);
  return useCallback(
    async (contractAddress: string, replyMessage: any, trace: any) => {
...
export function useDeleteCode() {
  const [{ app }, setSimulateApp] = useAtom(cwSimulateAppState);
  const [{ metadata }, setSimulationMetadata] = useAtom(
    simulationMetadataState
  );
  return useCallback(
    (codeId: number) => {
      setSimulateApp(({ app }) => {
...
export function getAddressAndFunds(
  chainId: string | undefined
): AddressAndFunds {
  switch (chainId) {
    case OraichainConfig.chainId:
      return {
        address: DEFAULT_ORAICHAIN_ADDRESS,
        funds: DEFAULT_ORAICHAIN_FUNDS,
      };
    case TerraConfig.chainId:
      return {
        address: DEFAULT_TERRA_ADDRESS,
        funds: DEFAULT_TERRA_FUNDS,
      };
    ...
    case InjectiveConfig.chainId:
      return {
        address: DEFAULT_INJECTIVE_ADDRESS,
        funds: DEFAULT_INJECTIVE_FUNDS,
      };
    default:
      return {
        address: DEFAULT_ORAICHAIN_ADDRESS,
...
export const convertCodeIdToCodeName = (codeId: string, codes: Codes = {}) => {
  const code = Object.values(codes).find(
    (code) => code.codeId.toString() === codeId
  );
  return code?.name;
};
