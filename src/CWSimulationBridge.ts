...
export default class CWSimulationBridge {
  ...
  public codes = this.app.store.db.lens<Record<number, CodeInfoEx>>(
    "wasm",
    "codes"
  );
  public contracts = this.app.store.db.lens<Record<string, ContractInfoEx>>(
    "wasm",
    "contracts"
  );
  ...
  recreate(options: CWSimulateAppOptions) {
    ...
    this.codes = this.app.store.db.lens<Record<number, CodeInfoEx>>(
      "wasm",
      "codes"
    );
    this.contracts = this.app.store.db.lens<Record<string, ContractInfoEx>>(
      "wasm",
      "contracts"
    );
    this.sync();
...
function compareManyCodes(
  ...
  for (const key of lkeys) {
    if (!compareCodes(lhs[key], rhs[key])) return false;
  }
  return true;
}
